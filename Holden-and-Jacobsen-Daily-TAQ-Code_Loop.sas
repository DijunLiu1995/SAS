/* HOLDEN AND JACOBSEN DAILY TAQ CODE 2018-03-16 
   ...
   cleans the data, creates the official complete National Best Bid and Offer
   (NBBO), computes standard liquidity measures, and writes output files to
   your WRDS cloud folder. It was last updated on March 16, 2018.
   ...
   adapted to work with WRDS and updated for the switch to nanoseconds.

   Our code creates the nine WORK files listed below in the WRDS Cloud. In
   the last step, you can choose which files you want to write to your WRDS
   cloud folder. By default, the last three files are written there.

      (1) Raw data files containing DTAQ data from WRDS: 
          "DailyNBBO" contains NBBO data
          "DailyQuote" contains quote data
          "DailyTrade" contains trade data

          Importantly, the "DailyNBBO" file does NOT contain the complete
          NBBO. When one exchange has both the best bid and best offer it
          is only noted in the "DailyQuote" file, not the "DailyNBBO" file.
          Our code combines data from both files to construct the official 
          complete NBBO (see the file below).

      (2) Intermediate data files:
          "OfficialCompleteNBBO" contains the official complete NBBO
          "TradesandCorrespondingNBBO" contains trades and the corresponding
              NBBO from the prior microsecond 
          "BuySellIndicators" adds buy/sell indicators based on three 
      ...
      (3) Output files containing standard liquidity measures:
          "QuotedSpreadsandDepths" contains Quoted Spreads and Depths
          "EffectiveSpreads" contains Effective Spreads
          "RealizedSpreadsandPriceImpacts" contains Realized Spreads and 
              Price Impacts that are aggregated based three conventions:
              Ave = simple average, DW = dollar-weighted, SW = share-weighted
   
   Step-by-step instructions for running this WRDS Cloud program using PuTTY
   and WinScp are available at: www.kelley.iu.edu/cholden/instructions.pdf
   ...
   We thank Henk Berkman, Adam Booker, Charles Collver, Marco Eugster, 
   Ruslan Goyenko, Mehrdad Samadi, David Xu, and Zhong Zhang for helpful 
...
rsubmit;
option notes;
PROC PRINTTO LOG='/home/lancaster/atimsal/Daily_loop.log' NEW;
RUN;
...
%macro ntq_daily_dataset_list(lib=,type=,begyyyymmdd=,endyyyymmdd=);
...
%if %sysfunc(exist(&lib..&type._&yyyymmdd)) %then &lib..&type._&yyyymmdd;
 %end;
%mend;

%let in_symbols='AAPL','IBM';

    /* Retrieve NBBO data */
    data DailyNBBO;

        /* Enter NBBO file names in YYYYMMDD format for the dates you want */
        set %ntq_daily_dataset_list(lib=nbbo,type=nbbom,begyyyymmdd=20180426,endyyyymmdd=20180427) open=defer;

		/* Enter company tickers you want */
        where sym_root in (&in_symbols.) 
    ...
    data DailyQuote;
        ...
        set %ntq_daily_dataset_list(lib=cq,type=cqm,begyyyymmdd=20180426,endyyyymmdd=20180427) open=defer;
    ...
    data DailyTrade;
        ...
        set %ntq_daily_dataset_list(lib=ct,type=ctm,begyyyymmdd=20180426,endyyyymmdd=20180427) open=defer;
